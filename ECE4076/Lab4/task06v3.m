% VERSION 3 SCALES THE X and Y PROPERLY WHEN DISPLAYING 
...
%     subplot(3,1,2);
    ...
    % figure
    hold on
    scatter3(xy(1,:) .* depthz(:)',(xy(2,:)).*depthz(:)', depthz(:),...
        ones(matched,1)*scatsizes(imB-1), clrs(imB-1,:),futuredots(imB-1,1));
%     axis([0, 640, 0, 480]);
    hold off
...
%     subplot(3,1,3);
    ...
    % figure
    hold on
    scatter3((xy(1,:)).*scaleddepthz(:)',(xy(2,:)).*scaleddepthz(:)',scaleddepthz(:), ...
        ones(matched,1)*scatsizes(imB-1), clrs(imB-1,:),futuredots(imB-1,1));
%     axis([0, 640, 0, 480]);
    hold off
...
% xlabel('Image Frame X')
% ylabel('Image Frame Y')

% subplot(3,1,2);
subplot(2,1,1);
title("Computed depths of keypoints. Plotted from Img1's Origin (rotate to see)")
...
% subplot(3,1,3);
subplot(2,1,2);
title("Computed scaled depths. Plotted from Img1's Origin")
legend('Img1&2','Img1&3','Img1&4');
xlabel('x Coords')
ylabel('y Coords')
zlabel('z Coords')