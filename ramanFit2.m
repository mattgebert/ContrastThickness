...
for index = 1:length(ramanfiles)
    file = ramanfiles(index);
    filename = ['2018-07-12/Batch04/' file.name];
    outputname = ['Output/Raman/' replace(filename, '.txt', '.jpg')];
    
    newdirs = split(outputname,'/');
    newdirname = strjoin(newdirs(1:end-1),'/');
    mkdir(newdirname)
    
    A = importdata([ramanpath filename]);
...
%     f1 = @(param,xdata) param(1,3) ./ (1 + ((param(1,1) - xdata)./ param(1,2)).^2) + param(2,1); %Anonymous function
    ...
    % Plot fits
    for i = 1:3    
        fitfig = figure('Name',['Lorentzian Fits ' filename ' - ' int2str(2^(i-1)) ' peaks'],'units','normalized','outerposition',[0 0 0.8 0.8]); % 'pos',[10,10,480*2,480*2]
        hold on
        plot(shift, amp,'.b')
        p_i = p{i};
        s = size(p_i);
    ...
    %     TODO GET THIS BIT WORKING ON RESIDUAL % Later...
        ...
        saveas(fitfig, replace(outputname, '.jpg', ['_Lorentzians_' int2str(2^(i-1)) '.jpg'] ));
        close(fitfig);
    end
    ...
    % Plot Full Spectrum
    ramanSpec = figure('Name',['Raman Spectrum ' filename],'units','normalized','outerposition',[0 0 0.8 0.8]); % 'pos',[10,10,480*2,480*2]
    plot(rShift,rAmp)
    xlabel('Raman Shift (cm^{-1}')
    ylabel('Intensity (AU)')
    title('Raman Spectrum')
    
    saveas(ramanSpec, replace(outputname, '.jpg', '_RamanSpectrum.jpg'));
    close(ramanSpec);
end